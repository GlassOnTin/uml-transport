...
if [ "$rc" = 0 ] && [ -f "$O/linux" ]; then
    # An app process runs under zygote's seccomp filter, which kills
    # set_robust_list and rseq with an uncatchable SIGSYS -- glibc (and
    # bionic's set_robust_list) issue both at startup, so an unpatched
    # binary dies with exit 159 before printing anything. Neuter the svc
    # sites post-link, then gate: fail the build if any site survives.
    python3 "$HERE/patch-glibc-seccomp.py" "$O/linux"
    if python3 "$HERE/check-app-seccomp.sh" "$O/linux"; then :; else
        echo "build-bionic.sh: seccomp check FAILED -- refusing to publish" >&2
        exit 1
    fi
